...
class ELM_AE():
    def __init__(self, Q, P, N, device, seed, lcg_model):
        self._input_size = P
        self._h_size = Q
        self._device = device
        # self._lambda = 0.001
        ...
        self._alpha = my_orth(LCG(self._h_size, self._input_size, seed, lcg_model)).to(device)
...
def LCG(m, n, seed, lcg_model):
    ...
    #         V[x] = (a*V[x-1]+b) % c
        ...
        with open(f'code_RNN/weights/{lcg_model}.pkl', 'rb') as f:
            V = pickle.load(f)
            f.close()      
        V = V[seed:L+seed]
